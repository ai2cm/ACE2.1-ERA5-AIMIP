...
def _get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        required=True,
        help=(
            "Directory (local or GCS) containing raw inference results. "
            "This is the output directory of run-ace-inference.sh, containing "
            "one subdirectory per simulation named after the simulation."
        ),
    )
    parser.add_argument(
        ...
        default=None,
        help=(
            "GCS destination for processed results. Required unless --skip-gcs-upload "
            "is set."
        ),
    )
    parser.add_argument(
...
def main(args: argparse.Namespace) -> None:
    if not args.skip_gcs_upload and args.processed_results_dir is None:
        raise ValueError(
